<template>
    <div>
        <v-card>
            <v-card-title primary-title>
                <div>
                    <h3 class="headline ma-2">
                        <v-icon>person</v-icon>
                        {{userName}}
                    </h3>
                    <h3 class="headline ma-2">
                        <v-icon>email</v-icon>
                        {{userEmail}}
                    </h3>
                </div>
            </v-card-title>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-dialog v-model="dialog" persistent max-width="550">
                    <v-btn slot="activator" dark flat color="orange">Изменить данные</v-btn>
                    <v-card>
                        <v-card-title class="headline">Изменить данные?</v-card-title>
                        <v-card-text>
                            <v-alert :value="getError" type="warning">
                                {{getError}}
                            </v-alert>
                            <v-form v-model="valid">
                                <v-text-field prepend-icon="email" name="login" label="Почта"
                                              type="email" required
                                              v-model="email"
                                              :rules="emailRules"></v-text-field>
                                <v-text-field id="password" prepend-icon="lock" name="password"
                                              label="Пароль" type="password" required
                                              v-model="password"
                                              :rules="passwordRules"></v-text-field>
                                <h3>Изменить:</h3>
                                <v-radio-group v-model="changeType">
                                    <v-radio label="Имя" value="name"></v-radio>
                                    <v-text-field prepend-icon="person" v-if="changeType =='name'"
                                                  name="newname" label="Новое имя"
                                                  type="text" required
                                                  v-model="newname"
                                                  :rules="nameRules"></v-text-field>
                                    <v-radio label="Почта" value="email"></v-radio>
                                    <v-text-field prepend-icon="email" v-if="changeType =='email'"
                                                  name="newlogin" label="Новая почта"
                                                  type="email" required
                                                  v-model="newemail"
                                                  :rules="emailRules"></v-text-field>
                                    <v-radio label="Пароль" value="password"></v-radio>
                                    <v-text-field prepend-icon="lock"  v-if="changeType =='password'"
                                                  label="Новый пароль" name="newpassword"
                                                  type="password" required
                                                  v-model="newpassword"
                                                  :rules="passwordRules"></v-text-field>
                                </v-radio-group>
                            </v-form>
                        </v-card-text>
                        <v-card-actions>
                            <v-spacer></v-spacer>
                            <v-btn color="green darken-1" flat @click="dialog = false">Отмена</v-btn>
                            <v-btn color="green darken-1" flat
                                   @click.prevent="changeUserData"
                                   :disabled="getProcessing || !valid">Изменить
                            </v-btn>
                        </v-card-actions>
                    </v-card>
                </v-dialog>

            </v-card-actions>
        </v-card>

    </div>
</template>
<script>
    import {mapGetters} from 'vuex'
    export default{
        data(){
            return {
                dialog: false,
                changeType: '',
                email: '',
                password: '',
                newname: null,
                newemail: null,
                newpassword: null,
                valid: false,
                nameRules: [
                    (v) => !!v || 'Введите имя',
                    (v) => (v && v.length >= 2) || 'Имя - минимум 2 символа'
                ],
                emailRules: [
                    (v) => !!v || 'Укажите почту',
                    (v) => /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(v)
                    || 'Некорректный адрес'
                ],
                passwordRules: [
                    (v) => !!v || 'Введите пароль',
                    (v) => (v && v.length >= 6) || 'Пароль - минимум 6 символов'
                ],
            }
        },
        computed: {
            ...mapGetters(['userName', 'userEmail', 'getProcessing', 'getError'])
        },
        methods: {
            changeUserData(){
                this.$store.dispatch('change_user_profile_data', {
                    email: this.email,
                    password: this.password,
                    newName: this.newname,
                    newEmail: this.newemail,
                    newPassword: this.newpassword,
                    changeType: this.changeType
                })
            }
        }
    }
</script>